...
# Load the Excel file
file_path = "C 50.xlsx"  # Replace with your actual file path
...
for freq, values in frequency_data.items():
    ...
    plt.text(
        freq, max_value + 0.065,  # Slight offset above the thread
        f"{max_value:.3f}",
    ...
    plt.text(
        freq, min_value - 0.065,  # Slight offset below the thread
        f"{min_value:.3f}",
        ha='center', va='top', fontsize=10, color="black"
    )

# Customize the plot
plt.title("Speech Clarity (C50) with different AM Materials as Ceiling", fontsize=18)
plt.xlabel("Frequency (Hz)", fontsize=16)
plt.ylabel("C50 in dB", fontsize=16)
...
plt.ylim(8.00, 15.00)  # Set y-axis limits
plt.yticks(np.arange(8.00, 15.50, 0.50), fontsize=12)  # Set ticks at intervals of 0.025
...
# Show the plot
output_file = "C50_Threads_Narrow_YAxis.png"
